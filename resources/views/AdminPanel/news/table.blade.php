                ...
                <td>
                    <span class="badge badge-light-success fs-7 fw-bold cursor-pointer symbol symbol-35px">
                        <img onerror="this.onerror=null;this.src='{{ asset('assets/media/svg/files/blank-image.svg') }}'"
                            src="{{ $new->image }}" class="rounded-3" a>
                    </span>
                    <span class="badge badge-light-success fs-7 fw-bold">{{ $new->title }}</span>
                </td>
                <td>
                    <span class="full-description d-none">{!! $new->description !!}</span>
                    <button type="button" class="btn btn-link fs-6 fw-bold read-more">Read More</button>
                </td>
                <td>
                    @if (auth()->user()->can('update news'))
                        <a href="{{ route('news.edit', $new->id) }}" class="btn btn-sm btn-light me-2">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                    @endif

                    @if (auth()->user()->can('delete news'))
                        <a data-url="{{ route('news.destroy', $new->id) }}" type='button'
